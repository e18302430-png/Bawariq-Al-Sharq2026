...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (hash === "#supervisor" || hash === "#admin" || hash === "#bawariq" || hash === "#support") return;

    const savedStep = localStorage.getItem("bawariq_courier_step");
    const savedId = localStorage.getItem("bawariq_courier_id");
    const savedInfo = localStorage.getItem("bawariq_courier_reg_info");
    ...
    if (savedInfo) { try { setRegistrationInfo(JSON.parse(savedInfo)); } catch (e) {} }
    if (savedDate) setScheduledDate(savedDate);
    if (savedTime) setScheduledTime(savedTime);
    if (savedName) setCourierName(savedName);
    if (savedStep && savedId) setStep(Number(savedStep));
  }, []);

  useEffect(() => {
    const checkHash = () => {
      const hash = window.location.hash;
      if (hash === "#admin" || hash === "#bawariq") { setIsAdminMode(true); setIsSupportMode(false); setIsSupervisorMode(false); }
      else if (hash === "#support") { setIsSupportMode(true); setIsAdminMode(false); setIsSupervisorMode(false); }
      else if (hash === "#supervisor") { setIsSupervisorMode(true); setIsAdminMode(false); setIsSupportMode(false); }
      else { setIsAdminMode(false); setIsSupportMode(false); setIsSupervisorMode(false); }
    };
    checkHash();
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data.success) throw new Error(data.error || "لم نجد أي طلب مطابق.");
      const { courier } = data;
  ...
  const handleRegisterSuccess = (id: string, info: typeof registrationInfo) => {
    setCourierId(id); setCourierName(info.name); setRegistrationInfo(info);
    setStep(2);
    ...
    <div className="min-h-screen flex flex-col bg-slate-950 font-sans tracking-tight text-slate-100 selection:bg-amber-500 selection:text-slate-950 overflow-x-hidden">
      ...
      <main className="flex-grow max-w-6xl mx-auto w-full px-4 py-8 md:py-12 space-y-12">
        {isAdminMode ? (
          ...
          supervisorData ? (
            <SupervisorDashboard supervisor={supervisorData} password={supervisorPassword}
          ...
          <div className="space-y-10">
            {step > 0 && (
              <div className="bg-slate-900/30 border border-slate-900 p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2.5">
                  <div className="hidden sm:flex items-center justify-center w-8 h-8 rounded-full bg-amber-500/10 text-amber-400 font-extrabold text-xs">{Math.floor(step)}</div>
                  <div>
                    ...
                    <h3 className="text-sm font-bold text-white">
                      ...
                      {step === 2 && "شروط وضوابط العمل والعقد التشغيلي"}
                      {step === 2.5 && "جدولة موعد المقابلة"}
                      {step === 3 && "الحصول على تصريح الدخول والبدء"}
                    </h3>
                  </div>
                </div>
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 text-xs font-semibold">
                  {[["الاسم والتطبيق", 1], ["العقد والتسعيرة", 2], ["حجز الموعد", 2.5], ["رقم التصريح", 3]].map(([label, s]) => (
                    <React.Fragment key={s}>
                      <span className={`px-2.5 py-1 rounded-md text-[10px] ${step >= (s as number) ? "bg-amber-500 text-slate-950 font-bold" : "bg-slate-900 text-slate-600"}`}>{label}</span>
                      {s !== 3 && <span className="text-slate-700">←</span>}
                    </React.Fragment>
                  ))}
                  <button onClick={handleReset} className="mr-2 px-3.5 py-1.5 text-[10px] font-extrabold text-amber-500 border border-amber-500/20 bg-slate-950/80 rounded-md cursor-pointer">
                    البدء من الأول ↺
                  </button>
                </div>
              </div>
                  ...
                  {showLookup && (
                    <div className="max-w-md mx-auto bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-2xl space-y-4 text-right">
                      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                        <button type="button" onClick={() => setShowLookup(false)} className="p-1 hover:bg-slate-800 rounded-lg text-slate-500 cursor-pointer"><X className="w-4 h-4" /></button>
                        <h4 className="text-xs font-black text-white flex items-center gap-1.5"><Search className="w-4 h-4 text-amber-400" /><span>الاستعلام عن طلب</span></h4>
                      </div>
                      <form onSubmit={handleLookup} className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input type="text" required value={lookupQuery} onChange={e => setLookupQuery(e.target.value)} placeholder="مثال: 0512345678 أو 1024354228"
            ...
            {step === 1 && (
              <div className="max-w-3xl mx-auto space-y-6">
                <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                  ...
                  <span className="text-xs text-slate-500 font-bold">يرجى ملء الحقول المطلوبة</span>
                </div>
            ...
            {step === 2 && (
              <div className="max-w-3xl mx-auto">
                <PricingAndAgreement courierId={courierId} courierName={courierName} onSuccess={handleAgreementSuccess} />
              </div>
            )}
