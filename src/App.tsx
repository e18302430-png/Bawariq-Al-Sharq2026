...
export default function App() {
  ...
  // تحديد الوضع من الـ hash أولاً قبل أي شيء
  const getInitialMode = () => {
    const hash = window.location.hash;
    return {
      isAdmin: hash === "#admin" || hash === "#bawariq",
      isSupport: hash === "#support",
      isSupervisor: hash === "#supervisor",
    };
  };

  const initialMode = getInitialMode();
  const [isAdminModeInit] = useState(initialMode.isAdmin);
  const [isSupportModeInit] = useState(initialMode.isSupport);
  const [isSupervisorModeInit] = useState(initialMode.isSupervisor);
  ...
  // قراءة localStorage فقط لو مش في صفحة مشرف أو أدمن
  useEffect(() => {
    const hash = window.location.hash;
  ...
  const handleRegisterSuccess = (id: string, info: typeof registrationInfo) => {
    ...
    localStorage.setItem("bawariq_courier_phone", (info as any).phone || "");
  };
