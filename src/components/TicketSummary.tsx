...
export default function TicketSummary({ courierId, info, scheduledDate, scheduledTime, onReset }: TicketSummaryProps) {
  ...
  const fetchLiveStatus = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/courier-profile/${courierId}`);
      if (res.ok) {
        let data;
        try {
          data = await res.json();
    ...
    <div className="space-y-8 max-w-2xl mx-auto print:bg-white print:text-slate-900" id="receipt-ticket-container">
      <div className="text-center space-y-3">
        ...
        <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
          تمت جدولة موعدك آلياً وإضافته إلى نظام الموارد البشرية لـ بوارق الشرق.
        </p>
      </div>
      ...
      <div className="relative border border-amber-500/30 bg-slate-950 rounded-2xl overflow-hidden shadow-2xl shadow-amber-500/5">
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="bg-slate-900/50 border border-slate-800/60 rounded-xl p-4 space-y-4">
            <div className="flex justify-between items-center flex-wrap gap-2">
              ...
              <button onClick={fetchLiveStatus} disabled={loading} className="px-2 py-1 text-[9px] bg-slate-950 border border-slate-800 rounded hover:text-white flex items-center gap-1 cursor-pointer">
            ...
            <div className="relative py-2 pb-6 border-b border-slate-900/50">
              <div className="h-0.5 bg-slate-800 w-[80%] absolute top-4 left-1/2 -translate-x-1/2 rounded"></div>
              <div className="flex justify-between relative z-10 text-[9px] sm:text-[10px]">
                <div className="flex flex-col items-center gap-1.5">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[10px] ${localStatus === "جديد" || localStatus === "تمت المقابلة" || localStatus === "تم التفعيل" ? "bg-amber-500 text-slate-950 ring-4 ring-amber-500/10" : "bg-slate-800 text-slate-500"}`}>1</span>
                  <span className="font-extrabold text-slate-300">تقديم الطلب</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[10px] ${localStatus === "تمت المقابلة" || localStatus === "تم التفعيل" ? "bg-cyan-400 text-slate-950 ring-4 ring-cyan-400/10" : "bg-slate-800 text-slate-500"}`}>2</span>
                  <span className="font-extrabold text-slate-300">المقابلة الوجاهية</span>
                </div>
                <div className="flex flex-col items-center gap-1.5">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-[10px] ${localStatus === "تم التفعيل" ? "bg-emerald-400 text-slate-950 ring-4 ring-emerald-400/10" : "bg-slate-800 text-slate-500"}`}>3</span>
            ...
            {localStatus === "تم التفعيل" && appCode && (
              <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-xs space-y-2">
                <div className="flex items-center justify-between text-[11px] text-slate-300 font-bold border-b border-emerald-500/15 pb-1.5">
                  <span>🚀 تم التفعيل وتعيين كودك بنجاح:</span>
                  <span className="text-[10px] bg-emerald-500 text-slate-950 px-1.5 py-0.5 rounded uppercase font-black">مفعّل</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-400">كود المندوب:</span>
          ...
          <div className="grid gap-4 sm:grid-cols-2 bg-slate-900/40 border border-slate-800 p-4 rounded-xl text-sm">
            <div className="flex gap-3 items-center">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block">تاريخ المقابلة</span>
            ...
            <div className="flex gap-3 items-center">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block">وقت الحضور</span>
          ...
          <div className="border-t border-b border-dashed border-slate-800/80 py-4 grid gap-4 grid-cols-2 text-xs">
            <div>
              <span className="text-slate-500 block mb-0.5">الاسم الكامل:</span>
            ...
            <div>
              <span className="text-slate-500 block mb-0.5">المدينة:</span>
              <span className="font-bold text-slate-200">{info.city}</span>
            </div>
            <div>
              <span className="text-slate-500 block mb-0.5">التطبيقات:</span>
              <span className="font-bold text-amber-400">{info.apps.map(id => id.toUpperCase()).join(", ")}</span>
            </div>
          </div>
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-3.5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                <span className="text-[10px] text-slate-500 block">عنوان المقابلة:</span>
                <p className="text-slate-200 font-bold leading-normal">{OFFICE_INFO.address}</p>
              </div>
            </div>
            <a href={OFFICE_INFO.mapsLink} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-amber-400 font-extrabold flex items-center justify-center gap-1.5 transition-all text-[11px] shrink-0">
          ...
          <div className="bg-gradient-to-br from-emerald-500/10 via-slate-900/60 to-slate-950 border border-emerald-500/35 rounded-2xl p-6 space-y-5">
            <div className="flex gap-4 items-start">
              <div className="p-3 rounded-2xl bg-emerald-500 text-slate-950 shrink-0 shadow-[0_0_15px_rgba(16,185,129,0.3)] animate-pulse">
              ...
              <div className="space-y-1 text-right">
                <span className="text-[10px] bg-emerald-500 text-slate-900 px-2.5 py-0.5 rounded font-black inline-block mb-1">هام: التفعيل الفوري إلكترونياً ⚡</span>
                <h4 className="text-sm font-extrabold text-white">إرسال عبارة "جاهز لاستلام اليوزر" للبدء المباشر!</h4>
                <p className="text-xs text-slate-300 leading-relaxed">
                  أرسل رسالة واتساب بعبارة <strong className="text-emerald-400">جاهز لاستلام اليوزر</strong> إلى رقم المشرف الخاص بك مع إرفاق الهوية والرخصة والاستمارة.
                </p>
              </div>
            </div>

            <div className="bg-slate-950 border border-slate-900 rounded-xl p-4 space-y-3">
              <span className="text-xs font-bold text-slate-200 block border-b border-slate-900 pb-1.5">📂 المستندات المطلوبة:</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex-1 bg-slate-950 border border-slate-900 rounded-xl p-3 flex justify-between items-center">
                <span className="text-xs text-slate-400 font-bold">العبارة المفتاحية:</span>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-black text-amber-400 bg-amber-400/10 px-3 py-1 rounded-lg">جاهز لاستلام اليوزر</span>
                  <button type="button" onClick={() => { navigator.clipboard.writeText("جاهز لاستلام اليوزر"); alert("تم نسخ العبارة! 📋"); }} className="p-1.5 text-slate-500 hover:text-white bg-slate-900 rounded-md border border-slate-800 transition-colors text-[10px] font-bold">نسخ</button>
                </div>
              </div>
                ...
                className="px-5 py-4 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black rounded-xl transition-all text-xs flex items-center justify-center gap-2 shrink-0 shadow-[0_4px_15px_rgba(16,185,129,0.25)]"
              >
                <span>إرسال المستندات في الواتساب 🚀</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-2 bg-slate-900/10 p-4 rounded-2xl border border-slate-900">
            <div className="space-y-1.5 flex-1 text-center sm:text-right">
              <span className="text-xs font-extrabold text-white flex items-center gap-2 justify-center sm:justify-start">
                ...
                <span>امسح تصريح الدخول عند بوابة المكتب</span>
              </span>
              <p className="text-[11px] text-slate-400 leading-relaxed">هذا الباركود يسمح لك بالحصول على أولوية القبول والدخول السريع.</p>
            </div>
            <div className="flex-shrink-0 bg-white p-3.5 rounded-xl">
              <svg className="w-24 h-24 text-slate-950" viewBox="0 0 100 100" fill="currentColor">
                ...
                <path d="M35 15h10v10H35zM55 15h15v5H55zM45 35h15v10H45zM15 45h10v15H15zM35 55h20v5H35zM75 45h15v10H75zM70 70h15v15H70z" />
        ...
        <div className="bg-slate-900 border-t border-slate-800/80 p-4 text-center">
          <p className="text-xs text-slate-500 flex items-center justify-center gap-1.5 font-bold">
            ...
            <span>بوارق الشرق للخدمات اللوجستية - مرخص وموثق رسمياً</span>
          </p>
        </div>
      </div>

      <div className="flex flex-col sm:flex-row gap-4 items-center justify-center pt-2">
        <button onClick={handlePrint} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white px-5 py-3 rounded-xl hover:bg-slate-800 transition-all font-bold text-xs">
          <Printer className="w-4 h-4" />
          <span>اطبع تصريح الموعد</span>
        </button>
        <button onClick={onReset} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-slate-950 border border-slate-900 text-slate-400 hover:text-amber-400 px-5 py-3 rounded-xl hover:bg-slate-900/60 transition-all font-bold text-xs">
          <RefreshCw className="w-3.5 h-3.5" />
          <span>تقديم طلب مندوب جديد</span>
        </button>
      </div>
    </div>
  );
