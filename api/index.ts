...
app.get("/api/couriers/:id", async (req, res) => {
  try {
    ...
    if (c.supervisor_id && c.supervisor_id !== "direct") {
      ...
      if (sup.length) { supervisorName = sup[0].name; supervisorPhone = sup[0].phone || "0599612490"; }
    }
    res.json({ success: true, courier: { ...mapCourier(c), supervisorName, supervisorPhone } });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
