...
app.post("/api/agreement/accept", async (req: any, res: any) => {
  try {
    const { courierId, signature } = req.body;
    if (!courierId || !signature) return res.status(400).json({ success: false, error: "بيانات ناقصة" });
...
app.post("/api/admin/update-profile", async (req: any, res: any) => {
  try {
    ...
    if (!checkAdmin(password)) return res.status(403).json({ success: false, error: "غير مصرح" });
    const data = await db("couriers", "PATCH", {
      national_id: nationalId || "", iban: iban || "", car_plate: carPlate || "",
      vehicle_model: vehicleModel || "", app_courier_code: appCourierCode || "",
      activation_date: activationDate || "", admin_notes: adminNotes || "",
